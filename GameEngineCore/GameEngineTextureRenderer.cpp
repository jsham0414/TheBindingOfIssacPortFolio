...
void GameEngineTextureRenderer::CreateFrameAnimationCutTexture(const std::string& _AnimationName, const FrameAnimation_DESC& _Desc)
{
	...
	if (NewAni.Texture == nullptr) {
		MsgBoxAssert("텍스쳐를 찾을 수 없습니다.");
		return;
	}
	NewAni.FolderTexture = nullptr;
}

void GameEngineTextureRenderer::ChangeFrameAnimation(const std::string& _AnimationName, DWORD _Flag)
{
	std::string Name = GameEngineString::ToUpperReturn(_AnimationName);
	...
	if (CurAni != &(*FrameAni)[Name] || _Flag & FORCE) {
	...
	} else if (CurAni == &(*FrameAni)[Name] && _Flag & RESTART) {
		CurAni->Reset();
	}
}
...
bool GameEngineTextureRenderer::CurAnimationEnd() {
	return CurAni->bOnceEnd;
}

int GameEngineTextureRenderer::CurAnimationFrameIndex() {
